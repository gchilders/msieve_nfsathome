...
typedef struct {
	...
	uint64 lp_num_relations;   /* NFS prefilter relation count before 32-bit compaction */
	uint64 lp_num_ideals;      /* NFS prefilter ideal count before 32-bit compaction */
	uint32 lp_is_64bit;        /* NFS-specific LP intermediate uses 64-bit IDs */
	uint32 lp_needs_rmap;       /* source relation namespace exceeds uint32 */
} filter_t;
